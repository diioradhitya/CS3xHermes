...
class DutaMovie : MainAPI() {
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        return if (quality.isEmpty()) {
            val episode = Regex("Episode\\s?(\\d+)").find(title)
    ...
    private fun Element.toRecommendResult(): SearchResponse? {
        ...
        val poster = fixUrlNull(selectFirst("div.content-thumbnail img")?.attr("src"))
            ?.fixImageQuality()
        val quality = select("div.gmr-qual, div.gmr-quality-item > a")
            .text().trim().replace("-", "")
        val ratingText = selectFirst("div.gmr-rating-item")?.ownText()?.trim()
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val trailer =
            document.selectFirst("ul.gmr-player-nav li a.gmr-trailer-popup")?.attr("href")
        ...
        return if (tvType == TvType.TvSeries) {
            ...
            val episodes = document.select("div.vid-episodes a, div.gmr-listseries a")
                .mapNotNull { eps ->
                    ...
                    val rawTitle =
                        eps.attr("title").takeIf { it.isNotBlank() } ?: eps.text()
                    val cleanTitle =
                        rawTitle.replaceFirst(Regex("(?i)Permalink to\\s*"), "").trim()
                    val epNum =
                        Regex("Episode\\s*(\\d+)").find(cleanTitle)
                            ?.groupValues?.getOrNull(1)?.toIntOrNull()
