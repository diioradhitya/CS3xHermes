...
class Filmlokal : MainAPI() {
    override var mainUrl = "https://jorivet.com"
    ...
    private suspend fun loadMainUrlIfNeeded() {
        ...
        try {
            ...
            val array = json.optJSONArray("filmlokal")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        loadMainUrlIfNeeded()
        val document = app.get("$mainUrl?s=$query&post_type[]=post").document
    ...
    override suspend fun loadLinks(
        ...
        val mainIframe = document.selectFirst("div.gmr-embed-responsive iframe")
            ?.getIframeAttr()?.let { httpsify(it) }

        if (!mainIframe.isNullOrEmpty()) {
            loadExtractor(mainIframe, "$directUrl/", subtitleCallback, callback)
        }
